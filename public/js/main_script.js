...
function checkAnswer(index,selectedOption) {
    ...
    if(correctOptn=="A" && selectedOption==0) return 1;
    if(correctOptn=="B" && selectedOption==1) return 1;
    if(correctOptn=="C" && selectedOption==2) return 1;
    if(correctOptn=="D" && selectedOption==3) return 1;
    else return 0;
...
function calculate_score(weight,correctness,time){
   ...
   // let level_value =   correctness*(weight * (1-(Math.min(time,avg_time-0.01)/avg_time)));
    ...
    console.log("avg "+ (levels[0] + levels[1] + levels[2]))
    let que_level = Math.ceil(weight+((levels[0] + levels[1] + levels[2])/3));
   // console.log("5 % of : "+ (5 * 100)/(levels[0] + levels[1] + levels[2]))
    console.log("after adding 5 : "+ que_level)
    if(que_level<0 ) que_level =1;

    if(isCorrect==1){
        findQueFromLevel(weight+Math.ceil(Math.random()*5));
    }else{
        findQueFromLevel(weight-Math.ceil(Math.random()*5));
    }
